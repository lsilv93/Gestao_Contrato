...
const schema = z.object({
  ...
  version: obrigatorio("a versão", 30),
...
export async function salvarManual(_: Estado, form: FormData): Promise<Estado> {
  ...
  try {
    ...
    const m = await prisma.$transaction(async (tx) => {
      ...
      await auditar(tx, { usuario: u, action: idAtual ? "UPDATE" : "CREATE", entityName: "GoodPracticesManual", entityId: m.id, details: antes ? { antes, depois: m } : m });
      return m;
    });
